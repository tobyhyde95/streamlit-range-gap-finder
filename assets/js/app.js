;(function() {
    ...
    function initializeProjectManager() {
        ...
        if (projectManager.searchInput) {
            projectManager.searchInput.addEventListener('input', (e) => {
    ...
    async function loadProjects() {
        try {
            ...
            filterProjects(projectManager.searchInput ? projectManager.searchInput.value : '');
        } catch (error) {
    ...
    function filterProjects(searchTerm) {
        ...
        const filtered = allProjects.filter(project => {
            ...
            const descMatch = project.description && project.description.toLowerCase().includes(term);
    ...
    function toggleDescription(projectId) {
        const descElement = document.getElementById(`project-desc-${projectId}`);
        const toggleBtn = document.getElementById(`project-desc-toggle-${projectId}`);
    ...
    function renderProjectList(projects) {
        if (projects.length === 0) {
            projectManager.projectList.innerHTML = `
                <div class="text-center py-8 text-gray-500">
                    <p>No projects found. ${allProjects.length > 0 ? 'Try adjusting your search.' : 'Create your first project to get started!'}</p>
                </div>
            `;
            return;
        }
        ...
        projectManager.projectList.innerHTML = projects.map(project => {
            ...
            // Check if description is auto-generated (should be hidden)
            const isAutoGenerated = hasDescription && (
                project.description.toLowerCase().includes('auto-created project') ||
                project.description.toLowerCase().includes('auto created project') ||
                project.description.trim() === 'Auto-created project for analysis'
            );
            const shouldShowDescription = hasDescription && !isAutoGenerated;
            const descriptionId = `project-desc-${project.id}`;
            const toggleId = `project-desc-toggle-${project.id}`;
            
            return `
            <div class="border rounded-lg p-3 hover:bg-gray-50 transition-colors">
                <div class="flex justify-between items-start gap-3">
                    <div class="flex-1 min-w-0">
                        <h4 class="font-semibold text-base mb-1">${project.name}</h4>
                        ${shouldShowDescription ? `
                            <div class="text-sm text-gray-600 mb-1">
                                <span id="${descriptionId}" class="line-clamp-1">${project.description}</span>
                                <button id="${toggleId}" onclick="toggleDescription(${project.id})" 
                                        class="text-blue-600 hover:text-blue-800 text-xs ml-1 underline">
                                    Show more
                                </button>
                            </div>
                        ` : ''}
    ...
    async function deleteProject(projectId) {
        ...
        const confirmText = prompt('Are you sure you want to delete this project? This action cannot be undone.\n\nType DELETE to confirm:');
        
        if (confirmText !== 'DELETE') {
            if (confirmText !== null) {
                showNotification('Deletion cancelled. You must type DELETE exactly to confirm.', 'info');
            }
            return;
        }
