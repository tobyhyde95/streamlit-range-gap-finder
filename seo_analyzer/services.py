...
def run_full_analysis(our_file_path, competitor_file_paths, onsite_file_path, options_str, progress_reporter=None):
    ...
    try:
        ...
        run_category_overhaul = options.get('runCategoryOverhaul', True)
    ...
    if not competitor_only_df.empty:
        ...
        if has_onsite_data:
            ...
            if not prelim_keywords_df.empty and 'Opportunity Score' in prelim_keywords_df.columns:
    ...
    if not group_market_share_df.empty:
        ...
        group_market_share_report_raw = []
        group_market_share_keyword_map = {}
    ...
    if not core_group_market_share_df.empty:
        ...
        core_group_market_share_report_raw = []
        core_group_market_share_keyword_map = {}

    report("Generating Category Overhaul Matrix...", 4, total_steps)
    facet_potential_report_raw = [] # Initialize new report list
    if run_category_overhaul:
        # MODIFIED: Capture the dictionary of results
        ...
        print("Skipping Category Overhaul Matrix analysis as per user request.")
        category_overhaul_matrix_report_raw = []

    # MODIFIED: Add the new report to the final return dictionary
    return {
        ...
        "facetPotentialReport": facet_potential_report_raw, # <-- NEWLY ADDED
